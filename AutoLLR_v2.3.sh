...
PID: 1237   <sha1hash>    <<  process hiding as a kernel thread
More Info: hxxps://www.sandflysecurity.com/blog/detecting-linux-kernel-process-masquerading-with-command-line-forensics/#more-2777
ps auxww | grep \\[ | awk '{print \$2}' | xargs -I % sh -c 'echo PID: %; sha1sum /proc/%/exe''
**********" >> $computername/LiveResponseData/TriageSearches/kernel_proc_masq_2.txt
ps auxww | grep \\[ | awk '{print $2}' | xargs -I % sh -c 'echo PID: %; sha1sum /proc/%/exe' >> $computername/LiveResponseData/TriageSearches/kernel_proc_masq_2.txt
echo "ps auxww | grep \\[ | awk '{print \$2}' | xargs -I % sh -c 'echo PID: %; sha1sum /proc/%/exe''"

# SHA1 hash for each running process
echo "SHA1 hash for each running process
Checks all the system PIDs and calculates the SHA1 hash for each process executable.
EXAMPLE:
<sha1hash>  /proc/100/exe
<sha1hash>  /proc/101/exe
<sha1hash>  /proc/102/exe
sha1sum /proc/*/exe'
**********" >> $computername/LiveResponseData/TriageSearches/running_procs_sha1.txt
sha1sum /proc/*/exe >> $computername/LiveResponseData/TriageSearches/running_procs_sha1.txt
echo "sha1sum /proc/*/exe"
